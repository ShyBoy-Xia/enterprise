...
public class UserAction extends BaseController<User> {
	...
	public String uploadHeadImage(
		...
		User user = (User) RequestHolder.getSession().getAttribute("manage_session_user_info");//创建session和用户对象
		String realPath = request.getSession().getServletContext().getRealPath("/");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String ymd = sdf.format(new Date());
		String resourcePath = "attached/headPortrait/"+ymd+"/";//文件保存路径
		if(imageFile!=null){
			if(allowUpload(imageFile.getContentType())){
				String fileName = rename(imageFile.getOriginalFilename());//定义文件名
				int end = fileName.lastIndexOf(".");
				String saveName = fileName.substring(0,end);
				File dir = new File(realPath + resourcePath);//文件保存路径
				if(!dir.exists()){
					dir.mkdirs();
				}
